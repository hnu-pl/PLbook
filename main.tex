...
같은 3째줄의 \texttt{x\,<=\,n}의 \texttt{n}이 이제는 함수의 지역 변수로 선언된
...
앞서 소개한 람다식서에 $\lambda$와 같이 이름을 유효범위에 묶어주는
요소를 바인더(binder)라고 하는데 순수한 람다식의 문법구조에서
바인더는 $\lambda$가 유일하다. 실용적인 프로그래밍언어는 물론이고
이론적으로 연구되는 프로그래밍언어의 모형에서도 여러 종류의
바인더를 포함하는 경우가 드물지 않다.
