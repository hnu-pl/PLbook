...
JavaScript 언어의 정규식 등이 여러 종류가 있다.
...
지시적 의미구조(denotational semantics)

동작방식 의미구조(operational semantics)

작은걸음(small-step) 큰걸음(big-step)

axiomatic semantics 공리적 의미구조
...
TODO
유한오토마타(finite automata, FA) 혹은
유한상태기계(finite state machine, FSM) 이론적인 기계 혹은 가상머신
...
재차 강조하고 싶다. % TOOD 프로그래밍 언어 관련 원서 등 소개?
...
활용 사례가 가장 많은\footnote{단적으로, NASA에서도 활용한다. \url{https://ti.arc.nasa.gov/tech/rse/publications/vnv/}}
의미구조의 표현 방식이라 볼 수도 있다. 역사적으로도
공리적 의미구조의 기원이 되는 이론인 호어 논리(Hoare logic)\cite{Hoare69}의
발표와 함께 프로그램 검증(program verification) 분야가 본격적으로
시작하였다\cite{GSLeeHJKim20Hoare}고 보기 때문이다.

프로그램 검증을 통해 일반적인 검사(testing) 방식의 한계를 극복할 수 있다.
비유하자면 개미는 모두 검다는 성질을 확인하기 위해 10000마리의 개미를
확인해 보니 모두 검은 개미였다 할지라도 10001번째 잡은 개미가 붉은
개미일 수 있듯, 프로그램이 어떤 성질을 만족한다는 사례를 뒷받침하는
10000개의 테스트를 통과했더라도 또 다른 10001번째 입력으로 실행했을
때도 만족할 것이라는 보장이 없다는 것이 일반적인 검사 방식의 근본적인
한계이다. 프로그램을 실제로 실행하여 구체적인 입력 사례와 결과값을
확인하는 일반적인 검사 방식과 달리, 프로그램을 직접 실행하지 않고
그 설계도인 소스코드로부터 수학에서의 증명처럼 모든 가능한 경우에
대해 프로그램이 원하는 성질을 만족함을 보이는 것을 `프로그램 검증'이라
한다. 여기서 ``원하는 성질''이 어떤 내용인지 분명하고 자세하면서도
정확하게 표현하기 위해 고안된 형식논리(formal logic) 체계가 바로
호어 논리(Hoare logic)이다. 참고로, 프로그램이 의도한 바대로
동작하기 원하는 성질을 분명하고 자세히 표현했다는 뜻에서
그러한 내용을 프로그램의 `명세'(specification)라고도 부른다.
정리하면, 프로그램이 항상 그 명세(specification)대로 동작할 것임을
소스코드로부터 입증하는 것이 프로그램 검증(program verification)이다.

프로그램을 그 전제조건(precondition) 사후조건(postcondition)
호어 트리플(Hoare triple)
 {P} C {Q}
역사적으로 프로그램
검증(program verification)을


% https://formal.hknu.ac.kr/Publi/hoare.pdf
