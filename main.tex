...
순수 함수형 프로그래밍 언어로 게으른 값계산(lazy evaluation)에
...
`하스켈'이라는 프로그래밍 언어의 이름은 조합논리(combinatory logic)를
정립한 하스켈 커리의 이름을 따른 것이다. 하스켈이 함수형 프로그래밍 언어라고
...
지었을 가능성도 있을 것 같다. 이왕 주제를 벗어남 김에 흥미로운 일화를 하나
...
1950년대 말에 최초의 리습(LISP)을 만들었고, 로빈 밀너의 주도로 타입 유추를
...
확장한 `다형타입 람다계산법'(polymorphic $\lambda$-calculs, a.k.a. System~F)과
...
\begin{align*}
\mathcal{E} ~::=~& \bullet ~|~ \mathcal{E}_\lambda ~|~ \mathcal{E}_\Lambda \\
\mathcal{E}_\lambda ~::=~& TODO ~|~ TODO \\
\mathcal{E}_\Lambda ~::=~& TODO ~|~ TODO
\end{align*}
