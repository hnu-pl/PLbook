...
만족하는 정규식에 대한 미분\footnote{정규식 미분은
    $\partial_x r = r'$가 일반적인 표기지만
...
\end{enumerate}
이 절에서는 정규식의 미분을 동작방식 의미구조로 정의하는 데
집중할 것이다. 그 외에 브쇼조브스키가 정규식 미분에 대한
연구한 내용에 대해 조금 더 알아보려면 그의 연구성과를
...
\[ x^{-1} L ~=~ \{\,y \mid xy\in L\,\} \]
\begin{align*}
\partial \varnothing/\partial a = \varnothing &
\end{align*}

\[
\quad \partial \varepsilon/\partial a = \varnothing \]
\[    \partial a/\partial a = \varepsilon
\quad \partial b/\partial a = \varnothing (\text{단,}~ a\neq b) \]
\end{subfigure}
...
\[
\inference{
   \partial(r\,r^{*})\!/\partial a \Longmapsto r' }{
...
\]
\vspace*{-.5ex}
\[ \nu(\varnothing) = \varnothing \qquad\qquad
   \nu(\varepsilon) = \varepsilon \qquad\qquad
   \nu(a) = \varnothing \]
\[ \nu(r_1\VERT r_2) = \nu(r_1)\VERT\nu(r_2) \qquad
   \nu(r_1r_2) = \nu(r_1)\nu(r_2) \qquad
   \nu(r^{*}) = \varepsilon \]
...
브쇼조브스키가 정리한 정규식의 미분 공식은 다음과 같다.\\
\( \partial\varnothing/\partial a = \varnothing \qquad\qquad\quad
   \partial\varepsilon/\partial a = \varnothing \) \hfill
\( \partial a/\partial a = \varepsilon \qquad
   \partial b/\partial a = \varnothing \) (단, $a\neq b$)\\
\( \partial(r_1\VERT r_2)/\partial a = \partial r_1/\partial a \VERT
                                       \partial r_2/\partial a \)\hfill
\( \partial(r_1r_2)/\partial a = (\partial r_1/\partial a)r_2 \VERT
                                 \nu(r_1)(\partial r_2/\partial a) \)\\
\( \partial(r^{*})/\partial a = \partial rr^{*}/\partial a\varnothing \)\hfill
\( \partial r/\partial \varepsilon = r \qquad
   \partial r/\partial (ax) = \partial (\partial r/\partial a)/\partial x\)\\
정규식의 미분공식 및 큰걸음과 작은걸음 의미구조(그림\;\ref{fig:ReDerivBigSmallSem})에
나타나는 도우미 함수 $\nu:\mathcal{R}\to\mathcal{R}$의 성질은
$\varepsilon \in \llbracket r \rrbracket$인 경우에만
$\nu(r)\equiv\varepsilon$이며 그렇지 않은 경우에는
$\nu(r)\equiv\varnothing$이다.
