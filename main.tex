...
%\mathop{\textstyle{}\!\Rsh_{\!\!^{\partial\!\!\!}}\!/_{\!\!^\partial}}%
}
...
\( \varsigma(\varnothing\VERT r) = r = \varsigma(r \VERT\varnothing) \quad
   ...
   \varsigma(\varepsilon\,r) = r = \varsigma(r\,\varepsilon) \)\\[.6ex]
...
동작과정 의미구조의 이항관계는 보통 추론규칙의 형태로 정의된다.
...
\langle c^{*}\!,\,\varepsilon\rangle \) \vspace*{-1.5ex}
\end{quote}
위의 각 작은걸음 관계의 성립을 보이는 유도나무는 다음과 같다.
\begin{itemize}
 \item[\textcircled{\small 1}]
 $\inference*{
    \inference{
...
그 사이에 앞단의 작업을 마무리하며 진행되는 일련의 작업을
대략 의미분석(semantic analysis)이라 부른다 \cite{Aho2013compilers2nd}.
프로그램에서 이름(name) 혹은 식별자(identifier)가 유효범위(scope)에
맞게 사용되지, 또 계산식이 타입(type)에 맞게 활용되는지 등에 대한 검사도
여기에 포함되며, 이런 작업을 진행하며 얻은 정보를 문법나무의
속성(attribute)으로 추가하기도 한다. 예를 들면 문법나무의 요소마다
타입 검사 결과로 알게 된 타입 정보를 이후의 최적화 및 코드생성 등에
요긴하게 활용하기 위해 속성으로 추가해 놓기도 하는 것이다. 하지만
이것은 지금까지 대개의 프로그래밍 언어 구현을 그런 식으로 구성했다는
현상에 대한 분석일 뿐이다.

보통의 컴파일러에서 소위 `의미분석'으로 분류되는 모든 작업이 원론적으로
문법구조의 영역이 아닌 의미구조의 영역으로 딱 떨어지게 경계지을 수 있는
것은 아니다. 문맥자유문법(CFG)의 범위를 넘어 문맥의존문법(CSG)까지
활용하면 변수가 유효범위에 맞게 사용되었는지는 형식문법으로 표현이
가능하다고 알려져 있다.

\subsection{요약문법나무 + 이름의 유효범위 묶음 = 요약묶음나무}
요약문법나무(abstract syntax tree, AST)에다
이름을 유효범위에 묶어두기(name binding)까지 한 구조를
요약묶음나무(abstract binding tree, ABT)라고 한다.

\lipsum[1-2]

\subsection{프로그래밍 언어의 타입}
...
동작방식 의미론의 관점에서 표현해 보면 다음과 같다. 실행 전에 $e:\tau$,
...
가미되는 경우는 흔하다. 예를 들면 동적 디스패치를 기반으로 하는
...
동적 타입 언어 작성하듯 프로그래밍할 수 있으면서 필요에 따라서
