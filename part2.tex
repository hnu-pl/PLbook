...
적용 먼저 값계산에서 더 이상 계산이 진행되지 않는 값(value)은
$v ::= x \mid \lambda x.e \mid x~e$의 형태다.

인자의 계산을 우선시하는 `인자 먼저 값계산'(call-by-value evaluation)의
경우에는 인자가 값으로 계산된 다음에 $\beta$줄임이 진행되어야 하므로
값계산 맥락 뿐만 아니라 핵심 규칙인 $\beta$규칙도 다음과 같이 더 제한된
형태로 바꿔야 한다.
\begin{quote}
...
\end{quote}
인자 먼저 값계산에서 값(value)은 $v ::= x \mid \lambda x.e \mid x~v$의 형태다.
열린식을 제외하고 닫힌식만 고려한다면, 람다계산법에 대한
적용 먼저 값계산이든 인자 먼저 값계산이든 값(value)은 모두
함수요약식($\lambda x.e$)의 형태이다.
...
\]
\caption{타입없는 람다계산법의 적극적 값계산(eager evaluation, a.k.a.\\
	call-by-value evaluation)을 규정하는 큰걸음 의미구조
