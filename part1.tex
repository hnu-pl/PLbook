...
\section{자연언어와 프로그래밍언어에서 Syntax의 비중}
\label{sec:NatProgSyn}
\index{syntax}%
\index{구문}%
언어학에서 음성 기반의 자연언어를 다룰 때는
작은 단위부터 큰 맥락까지 아래와 같이 다섯 단계로 나눠 설명한다고 한다.
\begin{enumerate}\tightlist
    \item 음운론(phonology):
    소리의 단위인 음운을 다루는 이론.
    \item 형태론(morphology):
...
\begin{enumerate}[itemsep=0pt]
    \item 프로그래밍에 대해 소개하는 책에서 C나 Java 언어의 `문법'이라고 하면,
    이것은 영문에서 syntax를 옮긴 것인가, 아니면 grammar를 옮긴 것인가?
\end{enumerate}
...
\item ``정규문법이 아닌 문맥자유문법''으로 정의할 수 있다면
      항상 ``정규언어가 아닌 문맥자유언어''일까?
      그렇다면 그 이유를 설명하고, 아니라면 반례를 들라.
\item ``정규언어가 아닌 문맥자유언어'' 중에 정규문법으로 나타낼 수 있는 언어가 있을까?
      있다면 그 예를 들고, 없다면 그 이유를 설명하라.
\end{enumerate}

\section*{탐구과제}
\begin{enumerate}
\item C의 여러 줄 주석과 Rust의 여러 줄 주석은 대략
      \verb|/* ... */| 이런 모양으로 비슷해 보이지만,
      C의 경우와 달리 Rust의 여러 줄 주석은 정규문법으로는 처리할 수 없다.
      Rust의 여러 줄 주석이 C와 어떤 점이 다르기 때문에 이런 차이가 있는지
      조사하여 설명해 보라.
\item 실제 프로그래밍언어의 문법 중 대부분 요소는 문맥자유문법으로 표현하여
      처리할 수 있지만, 그 중 일부는 문맥자유문법의 범위를 벗어난 문맥의존문법
      혹은 다른 방법을 추가로 동원해야만 처리 가능한 것들도 있다.
      어떤 프로그래밍 언어의 어떤 요소가 그러한지 몇 가지 사례를 조사해 보라.
\end{enumerate}
...
\index{lexical structure|see{어휘구조}}%
...
이렇게 한줄로 이어진 문자열로부터
\index{어휘분석|see{lexical analysis}}%
...
참고로 그림\;\ref{sfig:GrammarAS}나 \ref{sfig:MathAS}의 표현은
생성규칙이나 BNF의 형태만을 빌렸을 뿐 엄밀한 의미에서는
형식언어이론에서 말하는 형식문법이라든가 BNF로 볼 수는 없다.
왜냐하면 형식언어이론에서는 문법의 대상이 되는 언어를 기호의 일차원적 나열인
문자열의 집합으로 정의하는데, 요약문법은 애초부터 요약문법나무(AST)만을 대상으로
할 뿐 일차원적 구조를 직접 처리할 수 없는 문법구조이기 때문이다.
마찬가지로 BNF 역시 구체적 문법을 표현하기 위한 표기법이다.
...
문자열 전체의 집다양하나타낸다고 소개했다. 그렇다면 그 멱집합
...
대표하는 정규식은 어떻게 작성해야 할까? 첫 글자는 1이어야차고로
...
\section*{연습문제}
\begin{enumerate}
 \item 일반적으로, 괄호가 나타나는 문법은 `구체적 문법'인가 `요약문법'인가?
 \item 이항 연산자 (예컨대, 수식에서 $+$, $-$, $\times$, $/$ 등)의
      우선순위나 결합방향(좌결합, 우결합 등)을 따지는 문법은
      `구체적 문법'인가 `요약문법'인가?
 \item 그렇다면 앞의 문제에서 우선순위나 결합방향을 따지지 않는
      종류의 문법에서는 왜 그러한 정보가 필요없는지 설명해 보라.
 \item 정규식의 구체적문법을 생성규칙 형태나 BNF 표현으로 작성해 보라.
 \item 정규식의 요약문법을 Haskell 데이터 타입으로 작성해 보라.
 \item 정규식 $\varepsilon$과 문자열 $\varepsilon$의 관계를 설명해 보라.
 \item 클레이니 스타와 관련된 공식($r^{*} \equiv \varepsilon\VERT r\,r^{*}$)을
       등호($=$)가 아닌 동치($\equiv$)로 본문에서 표기한 이유를 설명해 보라.
\end{enumerate}
...
\begin{enumerate}[itemsep=0pt]
 \item 정규식의 문법을 확장한 EBNF에 대해 알아보라.
       참고로, EBNF에는 다양한 표기 방식이 있으며 BNF와 약간 다른
타구과제   표기 방식을 택하는 경우도 있는데, 그림\;\ref{fig:EBNF}는
...
가로선이 생략된 것으로 이해해도 무방하다.

\subsubsection{큰걸음과 작은걸음 의미구조 비교}
\label{sssec:big-vs-small-Semantics}%
...
어디선가 하나로 합쳐져서 최종 결과는 어떻게든 같아지는 성질을 가진
...
호어 논리(Hoare logic) 외에도, 퀵정렬(Quicksort) 알고리즘과
...
\begin{itemize}[itemsep=0pt]
 ...
 \item 하나의 언어를 대상으로 하는 서로 다른 의미구조가 서로 어긋남
       없이 일관된 뜻을 나타내는 엄밀한 이론적 증명이 필요하다.
...
효율적인 알고리즘으로 처리하기 어렵기 때문에 지금까지의 대부분 프로그래밍
...
\subsection{이름의 유효범위(scope)와 납치(capture)}
...
타입은 실행 전의 프로그램식이나 계산의 진행 과정에 있는 계산식
...
`타입보존'(type preservation)이라고 한다. 또한 잘 설계된 정적
...
\begin{enumerate}
 ...
 \item C나 Java등 실제로 현업에서 널리 쓰이는 프로그래밍 언어에서
       타입보존(type preservation)이 성립하지 않는 사례가 있는지 조사해 보라.
\end{enumerate}
